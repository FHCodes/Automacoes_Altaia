...
class EricssonContentHandler(handler.ContentHandler):
	...
	def __init__(self, datetime, outdir, verbose, outSN, outMC, escape):
		...
		# Mongo Setup
		self._entitiesList=list()
		self.hostname = "10.121.149.23"
		self.port = 27017
		self.username = "collector"
		self.password = "naqos"
		self.isOpen = False     
		#name of mongo db
		self._dbNameBDU = "OI_BDU"
		#collections of mongo db
		self._collectionNameBDU = "INV_RADIO"
	
		try:
			#Open connection to mongo database using provided data
			enrich_client = pymongo.MongoClient(self.hostname, self.port)
			self.isOpen = True
		except Exception, e:
			print e
			#logger.warning("Could not get connection to database")
			self.isOpen = False

		if self.isOpen:
			try:
				#Get the cache DB where the document will be inserted
				enrich_db = enrich_client[str(self._dbNameBDU)]
				#print enrich_db
				#logger.warning("Connected to XXX")
			except Exception, e:
				print "Could not get database objects after establishing connection to database"
				self.isOpen = False
		...
		if self.isOpen:
			try:
				authentication=enrich_db.authenticate(self.username, self.password)
			except Exception, e:
				print e
				print "Authentication on database failed"
				self.isOpen = False
		
		if self.isOpen and authentication==True:     		
			self._BDUentitiesList=enrich_db[str(self._collectionNameBDU)].distinct("BTSNAME", {"VENDOR":"ERICSSON", "SITEOWNER" : "TIM", "TECHNOLOGY" : "LTE"}) #find({"VENDOR":"ERICSSON", "SITEOWNER" : "TIM", "TECHNOLOGY" : "LTE"}).distinct('BTSNAME')#, {"VENDOR":"ERICSSON", "SITEOWNER" : "TIM", "TECHNOLOGY" : "LTE"})
			for i in self._BDUentitiesList:
				self._entitiesList.append(str(i))
			#print "self._entitiesList= " + str(self._entitiesList)
			
		#self._entitiesList=["4GBAPNH5177"]#["4GBAAAU5101"]
		self._outDir = outdir
	...
	def startElementNS(self, name, qname, attrs):
		...
		# MeContext nodes signal the start of a file
			...
			MeContextID=str(attrs_no_ns["id"])
			
			#print 'name[1]= ' + str(name[1])
			#print "MeContextID= "+ str(attrs_no_ns["id"])
			...
			if MeContextID in self._entitiesList:
				#print type(self._entitiesList)
				#print self._entitiesList
				#print MeContextID
				#print "MeContextID= " + str(MeContextID) + " exist in entitiesList"
			...
			#	self._isMeContext = False
			#	self._outMC = False
			#elif MeContextID in self._entitiesList:
			#	self._isMeContext = False
			#	self._outMC = False
			#	print "MeContextID= " + str(MeContextID) + "  not in entitiesList"
