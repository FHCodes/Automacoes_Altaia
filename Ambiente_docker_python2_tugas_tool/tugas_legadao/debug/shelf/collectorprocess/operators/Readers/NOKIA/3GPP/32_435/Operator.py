...
__doc__ = \
    '''
    Nokia 3GPP 32.435 Performance XML reader

    Spec file syntax:
    <operation type="Readers" name="NOKIA.3GPP.32_435" use_end_time="True" process_me="False" minfo_optionals="False" jump_mtypes="True" intervalUnit="H/M/S" />
'''
...
class Operator(BaseOperator):
    ...
    def __init__(self, operationParams, baseObject={}):
        ...
        self.consolidate_data = False
        if "consolidate_data" in self.options:
            if self.options["consolidate_data"].upper() == "TRUE":
                self.consolidate_data = True

    def convert(self, familyObj=FamilyObject()):

        for document in familyObj.getDocuments():

            if "STARTTIME" in document["data"].keys():
                start_time_match = self.datetime_regex.match(document["data"]["STARTTIME"])
                if start_time_match is not None:
                    start_time_datetime = datetime.strptime(start_time_match.group(1), "%Y-%m-%dT%H:%M:%S")
                    document["data"]["STARTTIME"] = start_time_datetime.isoformat(sep=" ")
                else:
                    logger.error(
                        "Could not convert start time due to non matching regex with {0} ".format(document["data"]["STARTTIME"]), __file__)
                    continue
    ...
    def process(self, familyObj=FamilyObject(), baseObject={}):
        ...
        for file_path in files_to_process:
            ...
            logger.debug(
                "Reading Nokia Performance 3GPP 32.435 XML files '{0}' contents...".format(os.path.basename(logger_filepath)),
                __file__)
            try:
                ...
                for meas_info, namespace in getelements(file_path, "measInfo"):
                    ...
                    for meas_value in meas_info.iter(namespace + "measValue"):
                        measured_object_id = meas_value.attrib["measObjLdn"]
                        ...
                        try:
                            document["DISTNAME"] = measured_object_id.split(',')[0]
                        except:
                            document["DISTNAME"] = measured_object_id
                        document["MEASUREDOBJECTID"] = measured_object_id.replace('PLMN-PLMN/', '')
                        document["STARTTIME"] = begin_time
                        ...
                        if not self.consolidate_data:
                            ...
                            if unit_id not in consolidatedData.keys():
                                consolidatedData[unit_id] = dict()
                            infoPK = '{:s}_{:s}'.format(document['STARTTIME'], document['MEASUREDOBJECTID'])
                            if infoPK not in consolidatedData[unit_id].keys():
                                consolidatedData[unit_id][infoPK] = dict()
                            consolidatedData[unit_id][infoPK].update(document)

            except ET.ParseError as e:
                logger.error("Malformed NOKIA XML file '{0}', due to {1}".format(os.path.basename(logger_filepath), e.message),__file__)

            if self.consolidate_data:
                for unit_id in consolidatedData.keys():
                    for docKey in consolidatedData[unit_id].keys():
                        document = consolidatedData[unit_id][docKey]
                        familyObj.clearDocuments()
                        familyObj.setUnitID(unit_id)
                        # Prepare Namf Mediation envelope
                        try:
                        ...
                        data_document = {"dataTime": data_time, "granularitySec": document["INTERVAL"], "data": document}
