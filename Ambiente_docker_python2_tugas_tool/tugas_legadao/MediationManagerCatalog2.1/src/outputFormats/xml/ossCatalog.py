...
def process(data, config, parameters):
    ...
    for ossId in sorted(data.keys()):
        ...
        unit.set('ossId', unitObj.ossId.upper())
        unit.set('name', unitObj.name)
        ...
        for attributeId in attributeDict:
            if attributeId not in unitItemList:
                ...
                column.set('name', attributeObj.name)
