...
def process(docPath, fileConfigName, dataConfig, vendor):
	...
	for tableId in info.keys():
		...
		if unitOssId not in data.keys():
			unitObj = unit()
			unitObj.create(tableInfo['ossId'], tableInfo['id'], tableInfo['name'], tableInfo['desc'], tableInfo['hierarchy'])
			unitObj.tech = 'R'
		...
		for columnId in info[tableId]['items'].keys():
			...
			for attr in columnInfo.keys():
				if attr not in ['id', 'bdcolname', 'name', 'udn', 'desc', 'bdtype', 'dataType', 'dbn0type', 'dataUnit', 'multiplicity']:
