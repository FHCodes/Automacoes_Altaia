...
def process(docPath, fileConfigName, dataConfig, vendor):
    ...
    for fileName in fileConfig.keys():
        print("Nome do ficheiro: " + docPath + fileName)

        info = excelBase(docPath + fileName, fileConfig[fileName], catalogType).getExcelInfo()
        validateInformation(info, catalogType, vendor, dataConfig['rules'])
        adoc = fileName.replace('&', '_').split(' ')
        adoc = adoc[0] + '_' + adoc[1] + '_' + adoc[2]
        for tableId in info.keys():
            ...
            unitOssId = tableInfo['ossId'].replace('.0', '')

            tableObj = table()
            tableObj.create(tableInfo['id'].replace('.0', ''), tableInfo['ossId'].replace('.0', ''),
                            tableInfo['tableName'], tableInfo['udn'])

            if unitOssId.replace('.0', '') not in data.keys():
                unitObj = unit()
                unitObj.create(tableInfo['ossId'].replace('.0', ''), tableInfo['id'].replace('.0', ''),
                               tableInfo['name'], tableInfo['desc'], tableInfo['hierarchy'])
                tableInfo['adoc'] = adoc
                unitObj.addExtra('adoc', adoc, 'client')
                for attr in tableInfo.keys():
                    if attr not in ['id', 'ossId', 'name', 'udn', 'desc', 'tableName', 'hierarchy', 'tech',
                                    'disableHierarchy']:
                        unitObj.addExtra(attr, tableInfo[attr], 'oss')
                unitObj.tech = 'R'
                ...
                unitObj.tech = techDict[tableId]

            if tableInfo['id'].replace('.0', '') in unitObj.tables.keys():
                tableObj = unitObj.getTable(tableInfo['id'].replace('.0', ''))
                tableObj.tech = techDict[tableId]
            else:
                unitObj.addTable(tableInfo['id'].replace('.0', ''), tableObj)
            ...
            for columnId in info[tableId]['items'].keys():
                ...
                columnObj = column()
                columnObj.create(columnInfo['id'].replace('.0', ''), re.sub(r'\<|\>|&', '', columnInfo['name']),
                ...
                for attr in columnInfo.keys():
                    if attr not in ['unitId', 'dbn0type', 'id', 'bdcolname', 'name', 'udn', 'desc', 'bdtype',
                                    'dataType', 'dataUnit', 'multiplicity', '']:
...
def convertTech(cell, fileName, tech):
    ...
    if ('GSM' in fileName or 'GBTSFunction' in fileName) and '2G' not in tech:
        ...
        tech = tech + '5G'
    elif 'RFAFunction' in fileName and '5G' not in tech:
        tech = tech + '5G'
    elif 'GU' in fileName and '2G3G' not in tech:
        tech = tech + '2G3G'
    elif ' Node ' in fileName and '4G5G' not in tech:
        tech = tech + '4G5G'
