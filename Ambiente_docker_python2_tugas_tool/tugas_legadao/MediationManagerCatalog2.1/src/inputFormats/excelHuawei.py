...
def process(docPath, fileConfigName, dataConfig, vendor):
	...
	# To get missing data from hdx dir
	tName = raw_input('hdx tech name: ')
	path = raw_input('Path to hdx resource dir: ')
	if path != '':
		info = readHDXFile(tName, info, path)
	logger.debug('Finishid data processing')
	...
	for tableId in info.keys():
		...
		for columnId in info[tableId]['items'].keys():
			...
			# columnInfo['id'] = re.sub(r'\<|\>', '', columnInfo['id'])
