...
__authors__ = [
	...
	'Version 1.1: Marco Jeronimo <marco-a-jeronimo@alticelabs.com>'
]
...
# startText -> string to add in start of pattern
# hierarchySep -> char used to split multi-hierarchy's
"measuredObject3GPP_NSN":
[
	{
		"regex" : "PLMN-",
		"sepCharDoc" : "-",
...
def process(unitDict,config):
	for ossId in unitDict.keys():
		...
		hierarchyList = [measuredObject]

		if 'hierarchySep' in config:
			hierarchyList = re.split(config['hierarchySep'], measuredObject)
...
def createHierarchy(unitObj, config, measuredObject):
		...
		for attributeId in hierarchyId:
			...
			if ',(.+?)' in measuredObject[0]:
				pattern += measuredObject[0] + config['sepCharPattern'] + '(?P<' + attributeId.upper() + '>[^' + config['interChar'] + ']+?)'
			else:
