...
def process(unitDict, config):
	...
	for ossId in unitDict.keys():
		...
		for measObjBase in unitObj.measuredObject.split(';'):
					...
					for itDef in op['def']:
						if itDef['id'] == 'MEASOBJLDN':
							...
							for regexInfo in itDef['regexes']:
								if regexInfo['newUnit'] == info['newUnit']:
									regexFound = True
							...
							if not regexFound or not patternFound:
								regexItem = dict()
								regexItem['newUnit'] = info['newUnit']
		...
		for pos, primaryFamily in enumerate(unitSplitList[pattern]['familyList'][:-1]):
			...
			for secundaryFamily in unitSplitList[pattern]['familyList'][pos+1:]:
				...
				if matchPk not in matchList .keys():
					...
					#matchList[matchPk]['priority'] = len(list(set(primaryFamilyObj.getTable(pTableId).counters.keys()) & set(secundaryFamilyObj.getTable(stableId).counters.keys())))
					...
					if primaryFamily not in matchList[matchPk]['units']:
						matchList[matchPk]['units'].append(primaryFamily.upper())
				if secundaryFamily not in matchList[matchPk]['units']:
