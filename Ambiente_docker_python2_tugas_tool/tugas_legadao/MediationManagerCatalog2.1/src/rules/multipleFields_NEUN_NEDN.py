...
def process(unitDict, config):
    ...
    for ossId in unitDict.keys():
        ...
        for base_obj in measured_obj_list:
            ...
            measured_obj = re.sub(r',\s|/\s', ',', base_obj)  # clean ' ' and '/' -> ','
            ...
            if base_obj[0:len(sep_char_doc)] == sep_char_doc:
                ...
                # SAFE-MEASURE - SHOULD HAVE A STARTFIELD, DEFINES BY DEFAULT
            ...
            # specific for fento
            # additional pattern generated since NEUN and NEDN require additional ops -> measuredobject wont be affected since fieldName verification
            # 3rd field to brake should use the generic rule

            add_op_regex = ET.SubElement(operation, "regex")
            add_skip = False
            subnetwork_count = 0
            add_pattern = '^'
            ...
            for info in pre_obj_text:
                ...
                if fieldName == 'NEUN' and 'OpMode' in info:
                    add_skip = True

                if fieldName == 'NEDN' and 'SubNetwork' in info:
                    subnetwork_count += 1
                    obj_id += '_' + str(subnetwork_count)
                    if subnetwork_count == 4:
                        add_skip = True
                ...
                if has_startField:
                    ...
                    if not add_skip:
                        add_pattern += info + ('(?P<' + obj_id + '>[^' + block_in_regex + ']*?)' if block_in_regex != '' else '(?P<' + obj_id + '>.*?)')
                else:
                    ...
                    if info_fix != info:
                        ...
                        if not add_skip:
                            add_pattern += info_fix + sep_char_pattern + ('(?P<' + obj_id + '>[^' + block_in_regex + ']*?)' if block_in_regex != '' else '(?P<' + obj_id + '>.*?)')
                    else:
                        pattern += info + ('(?P<' + obj_id + '>[^' + block_in_regex + ']*?)' if block_in_regex != '' else '(?P<' + obj_id + '>.*?)')
                ...
                if not add_skip:
                    op_new_field = ET.SubElement(add_op_regex, "newField")
                    op_new_field.text = obj_id

                if add_skip and fieldName != 'NEUN':  # fieldName verification allows it to skip everything further if fieldName is NEUN
                    add_skip = False
            ...
            add_op_regex.set("pattern", add_pattern)
