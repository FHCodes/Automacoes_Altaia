...
def process(unitDict, config):
			...
			if tableObj.partitionOf != '':
				if tableObj.partitionOf.upper() != tableObj.typeId.upper():
					...
					if tableObj.partitionOf not in partitionerMapping.keys():
						...
						partitionerMapping[tableObj.partitionOf][tableId] = list()
	...
	for unitId in partitionerMapping.keys():
		unitObj = unitDict[unitId]
		if len(partitionerMapping[unitId].keys()) == 0:
			continue
		...
		if len(tableList) == 1:
			compareResult = sorted(list(set(partitionerMapping[unitId][tableList[0]]) & set(unitObj.tables[unitId].counters)))
			...
			mapByUnit[compareResultStr]['tables'].append(tableList[0])
		else:
			i = 1
		...
		primaryList = list(set([x.upper() for x in primaryFamilyObj.attributes.keys()]))
