...
def process(unitDict, config):
	config = json.load(open('./src/rules/configPTPtoOI.json'), object_pairs_hook=OrderedDict)
			...
			for i, newField in enumerate(hierObj.newFields):
				if newField.upper() in config['operations'].keys():
					for opObj in config['operations'][newField.upper()]:
						if (ossId in opObj['units'] and opObj['type'] == 'in') or (ossId not in opObj['units'] and opObj['type'] == 'out'):
							if newField.upper() in config['operations'].keys():
