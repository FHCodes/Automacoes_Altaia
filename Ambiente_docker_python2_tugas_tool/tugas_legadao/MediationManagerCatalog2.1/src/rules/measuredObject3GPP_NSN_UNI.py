...
def process(unitDict,config):
	for ossId in unitDict.keys():
		...
		insideFild = False
		word = ''
		for t in measuredObject:
			if t in ['(', ')']:
				insideFild = not insideFild
			if t == ',' and not insideFild:
				createHierarchy(unitObj, config, word)
				word = ''
			else:
				word += t
		createHierarchy(unitObj, config, word)
...
def createHierarchy(unitObj, config, measuredObject):
		...
		for attributeId in hierarchyId:
			...
			if ',(.+?)' in measuredObject[0]:
				pattern += measuredObject[0] + config['sepCharPattern'] + '(?P<' + attributeId + '>[^' + config['interChar'] + ']+?)'
			else:
				pattern += sep + measuredObject[0] + config['sepCharPattern'] + '(?P<' + attributeId + '>[^' + config['interChar'] + ']+?)'
