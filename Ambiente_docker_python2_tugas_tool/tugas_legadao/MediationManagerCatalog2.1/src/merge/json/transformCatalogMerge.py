...
def process(vendor, baseCatalogPath, newCatalogPath, numenclature, config):
	...
	baseCatalog = json.load(open('{0}/transformCatalog.json'.format(baseCatalogPath)))
	newCatalog = json.load(open('{0}/transformCatalog.json'.format(newCatalogPath)))

	logger.debug('#################################### Transform Catalog Merging Info ########################################')
	logger.debug('## Vendor: {:34s}                                                     ##'.format(vendor))
	logger.debug('## Output File:  {:80} ##'.format('loadingCatalog.json'))
	logger.debug('################################### Start Transform Catalog Merging ########################################')
	...
	logger.debug('#################################### End Transform Catalog Merging #########################################')
