            ...
            if ossId is None:
                ...
                for item in unit:
                    ...
                    if itemId == None:
                        ...
                        logger.warning(
                            '  * [Item] Duplicated Item \"{:2s}\" in Unit with ossId: \"{:2s}\" * '.format(itemId,
                                                                                                           ossId))
            else:
    ...
    for newUnit in newCatalog.findall('unit'):
        try:
            ...
            # Verify if newUnit is really new, if not compares it self with existing unit
                ...
                newUnitId = newUnit.get('ossId').upper()
                if newUnitId in data:
                    ...
                    for attr in config['unit'].keys():
                        try:
                            ...
                            elif attr == 'tech' and config['unit'][attr] == 'True':
                                #if unit.get(attr) != newUnit.get(attr):
                                    #logger.debug('valor da tech bc: {0} valor da tech nc: {1} - {2}'
                                                 #.format(unit.get(attr), newUnit.get(attr), newUnit.get('id')))
                                if '-' in unit.get(attr):
                                    newTechList = newUnit.get(attr).split('-')
                                    oldTechList = unit.get(attr).split('-')
                                    for nTech in newTechList:
                                        if nTech not in oldTechList:
                                            unit.set(attr, '{0}-{1}'.format(unit.get(attr), nTech))
                                else:
                                    if re.match(r'R(?:\dG)+\b', unit.get(attr)) and re.match(r'R(?:\dG)+\b', newUnit.get(attr)):
                                        oldTechComponents = set(re.findall(r'\d+G', unit.get(attr)))
                                        newTechComponents = set(re.findall(r'\d+G', newUnit.get(attr)))
                                        combineTech = oldTechComponents.union(newTechComponents)
                                        unit.set(attr, 'R' + ''.join(sorted(list(combineTech))))
                                    else:
                                        if unit.get(attr) != newUnit.get(attr):
                                            unit.set(attr, '{0}-{1}'.format(unit.get(attr), newUnit.get(attr)))
                                            
                                # logger.debug('Novo valor da tech: {0}'.format(unit.get(attr)))
                            elif attr == "ab" and config['unit'][attr] == 'True':
                                oldComponents = set()
                                if unit.get(attr) is not None:
                                    oldComponents = set(unit.get(attr).split(';'))
                                newComponents = set(newUnit.get(attr).split(';'))

                                combineAB = oldComponents.union(newComponents)
                                unit.set(attr, ';'.join(sorted((list(combineAB)))))
                    ...
                    for newItem in newUnit:
                        ...
                        if newItemId is None:
                            if not ET.tostring(newItem).upper() in data[newUnitId]['comment']:
                                unit.append(newItem)
                            elif not (ET.tostring(newItem).split(' id="')[1].split('"')[0]).upper() in data[newUnitId][
                                'comment']:
                                unit.append(newItem)
                        else:
                                        ...
                                        if attribute == 'v':
                                            try:
                                                ...
                                                if newItem.get('v') != data[newUnitId]['item'][newItemId].get('v'):
                                                    data[newUnitId]['item'][newItemId].set('v', baseVersion + '/' +
                                                                                           newItem.get('v').split('/')[
                                                                                               1])
                                            except:
                                                data[newUnitId]['item'][newItemId].set('v', baseCatalog.get(
                                                    'ossversion').split('/')[0] + '/' + newItem.get('v').split('/')[1])
                                        elif attribute == 'typeCust':
                                            if newItem.get(attribute) == 'STRING' and data[newUnitId]['item'][
                                                newItemId].get(attribute) == 'INTEGER':
                                                ...
                                                data[newUnitId]['item'][newItemId].set(attribute,
                                                                                       newItem.get(attribute))
                                        else:
                                            data[newUnitId]['item'][newItemId].set(attribute, newItem.get(attribute))
