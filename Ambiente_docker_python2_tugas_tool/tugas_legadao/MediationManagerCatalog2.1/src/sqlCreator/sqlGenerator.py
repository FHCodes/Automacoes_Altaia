...
def getModules():
	modules = dict()
	for importer, packageName, is_pkg in pkgutil.iter_modules(src.sqlCreator.modules.__path__):
		module = importer.find_module(packageName).load_module(packageName)
		if not is_pkg:
			modules[packageName] = module
		else:
			for sub_importer, sub_packageName, _ in pkgutil.iter_modules(module.__path__):
				modules["{0}.{1}".format(packageName, sub_packageName)] = sub_importer.find_module(sub_packageName).load_module(sub_packageName)
	return modules
