...
def generate(data, catalog, config):
	...
	for tableName in data['table']['new']:
		if tableName not in catalog.keys():
			logger.warning('[truncateTable] Table \"{:3s}.{:3s}\" is missing in catalog.'.format(config['schema'], tableName.lower()))
			continue
		sqlOutput += 'TRUNCATE TABLE {:3s}.{:3s};\n'.format(config['schema'], tableName.lower())

	if sqlOutput != '':
		writeToFile('postgres_truncateTable.sql', sqlOutput)
