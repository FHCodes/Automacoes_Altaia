...
def remover_apenas_linha_invcell():
    ...
    print(f"🧹 Iniciando a remoção apenas da linha <newField>INVCELL</newField> no arquivo: {filename}...")
    ...
    try:
        ...
        for unit in root.xpath("//*[local-name()='unit']"):
            ...
            if unit_id in ids_alvo:
                # Mudança principal: O XPath agora mira DIRETAMENTE na tag <newField> que tem o texto 'INVCELL'
                # e que está dentro de uma tag <operation>
                campos_invcell = unit.xpath(".//*[local-name()='operation']/*[local-name()='newField'][text()='INVCELL']")
                
                for campo in campos_invcell:
                    # Remove apenas a tag <newField> do elemento pai (que é a <operation>)
                    campo.getparent().remove(campo)
        ...
        if modificou_arquivo:
            ...
            print("ℹ️ Nenhuma tag <newField>INVCELL</newField> precisou ser removida nos IDs especificados.")
    ...
    if unidades_modificadas:
        print("\n📋 Lista de IDs das units onde a linha INVCELL foi removida:")
