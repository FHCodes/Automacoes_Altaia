...
def remover_operacao_invcell_por_id():
    ...
    print(f"🧹 Iniciando a remoção da operação INVCELL no arquivo: {filename}...")
    ...
    try:
        ...
        for unit in root.xpath("//*[local-name()='unit']"):
            ...
            if unit_id in ids_alvo:
                # Procura a tag <operation> que tem um filho <newField> com o texto INVCELL
                # O XPath ".." volta para pegar a tag <operation> inteira e não só o <newField>
                operacoes_invcell = unit.xpath(".//*[local-name()='operation'][*[local-name()='newField'][text()='INVCELL']]")
                
                for op in operacoes_invcell:
                    # Pega o elemento pai (que é a <unit>) e remove a <operation> dele
        ...
        if modificou_arquivo:
            ...
            print("ℹ️ Nenhuma operação de INVCELL precisou ser removida nos IDs especificados.")
    ...
    if unidades_modificadas:
        print("\n📋 Lista de IDs das units onde a operação INVCELL foi removida:")
        ...
        if nao_encontrados:
            print(f"\n⚠️ Os seguintes IDs da sua lista NÃO sofreram alteração (talvez não tivessem a operação INVCELL ou não existam no arquivo):")
            for n in sorted(nao_encontrados):
                print(f"  - {n}")

if __name__ == "__main__":
    remover_operacao_invcell_por_id()